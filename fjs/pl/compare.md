...
Są także jeszcze dwa inne systemy, których nie biorę pod uwagę. Po pierwsze – Sagittal – przeznaczony do notowania wszelkiej muzyki mikrotonalnej. Nie będziemy jednak go rozważać, ponieważ jest oparty na przedstawianiu wysokości, a więc niezależnie od dokładności wartości reprezentowanych przez znaki zawsze będzie niedokładny w zapisie JI, ponieważ interwały JI są nieskończenie dokładne, jak ułamki naturalne. Po drugie – Rational Comma Notation, autorstwa Davida Ryana – która jest zaskakująco podobna do FJS-u (chociaż te dwa systemy zostały wynalezione niezależnie!), potrafi zapisywać cały bezmiar JI, ale jest mało zdatna do użytku z powodu długaśnej notacji, braku nazw interwałów, oraz skomplikowanego algorytmu, a więc jej także nie rozważam.
